...
getStringVersions = function(docs, rn) {
...
exports.getOptionsDict = function(docs, identifier) {
  var docsLength = docs.length;
  for (var i = 0; i < docsLength; i++) {
    if (docs[i]['identifier'] == identifier) {
      return docs[i]['options'];
...
buildIDTree = function(docs, arr) {
  for (var i = 0; i < arr.length; i++) {
    if (arr[i] instanceof Array) {
      ...
      var idName = getIdentifierName(arr[i]);
      var idVersion = getStringVersion(arr[i]);
...
exports.getRecData = function(docs) {
  // local version?
  var recipeNames = getDocNames(docs);
  var recipeData = {};
  var rnLength = recipeNames.length;
  for (var i = 0; i < rnLength; i++) {
    // local version?
    var individualVers = getStringVersions(docs, recipeNames[i]);
    recipeData[recipeNames[i]] = individualVers;
  }
  return recipeData;
}

findParents = function(docs, identifier) {
  var parents = [];
  for (var i = 0; i < docs.length; i++) {
    for (var j = 0; j < docs[i]['children'].length; j++) {
      if (docs[i]['children'][j] == identifier) {
        parents.push(docs[i]['identifier']);
      }
    }
  }
  return parents;
...
findAllParents = function(docs, identifier) {
  var par = findParents(docs, identifier);
  if (par) {
    return par.concat(par.map(function(parID) {return findAllParents(docs, parID);}));
  }
  return [];
}

exports.getAllParents = function(docs, identifier) {
  var pars = findAllParents(docs, identifier);
  return undersc.flatten(pars);
}

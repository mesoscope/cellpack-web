...
$.post('/tabler', {'recipename': recipeName, 'recipevers': recipeVersion}, function(data) {
  ...
  var tableName = data['identifier'].split('-')[0];
  var tableVersion = data['identifier'].split('-')[1].split('_').join('.');
  ...
  $.each(Object.keys(data['options']), function(i, val) {
    recLabels = recLabels + '<th>'+ val + '</th>';
    recValues = recValues + '<td>' + data['options'][val] + '</td>';
    dyValues = dyValues + '<td><input type="text" name="' + val + '" val="" placeholder="' + data['options'][val] + '"></td>';
  });
